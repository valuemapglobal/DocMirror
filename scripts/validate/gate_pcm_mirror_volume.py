...
def measure() -> dict[str, int]:
    ...
    pcm_forensic = _payload_bytes(mirror)
    pcm_standard = _payload_bytes(mirror)
    return {
        "pcm_forensic_bytes": pcm_forensic,
        "pcm_standard_bytes": pcm_standard,
        "top_level_section_count": len(
            [key for key in ("mirror", "source", "document", "pages", "evidence", "regions", "blocks", "graph") if key in mirror]
        ),
    }
...
def main() -> int:
    ...
    if pcm_forensic <= 0:
        print("PCM mirror volume gate FAILED: vNext forensic payload is zero", file=sys.stderr)
        return 1

    if measured["top_level_section_count"] < 8:
        print("PCM mirror volume gate FAILED: vNext mirror is missing required top-level sections", file=sys.stderr)
        return 1

    print(
        "PCM mirror volume gate OK "
        f"(vNext forensic {pcm_forensic}, standard {pcm_standard})"
    )
    return 0
