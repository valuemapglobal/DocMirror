...
"""Design 19 G2/G3 — fail CI when production code reads deprecated mirror JSON paths."""
...
G2_ALLOWED_SUFFIXES = {
    ...
    "models/mirror/legacy_project.py",
...
G3_ALLOWED_SUFFIXES = {
    ...
    "core/ocr/page_canvas/build.py",
    "models/entities/parse_result.py",
    "core/bridge/parse_result_bridge.py",
    "core/pipeline/handlers/scanned_page.py",
...
def check_g3() -> list[str]:
    ...
    for path in _scan_py(DOCMIRROR):
        ...
        for lineno, line in enumerate(text.splitlines(), start=1):
            if "scanned_local_structure_evidence" in line and (
                "structures" in line or '.get("structures")' in line
            ):
...
def main() -> int:
    ...
    if errors:
        print("PCM legacy reference gate FAILED:", file=sys.stderr)
        for err in errors:
            print(f"  - {err}", file=sys.stderr)
        print(
            "Allowed readers: page_access, legacy_project, domain_access (G3); "
            "see docs/design/19_page_centric_mirror_unification_execution_plan.md §5.1",
            file=sys.stderr,
        )
        return 1
    print("PCM legacy reference gate OK (G2 micro_grids + G3 local_structure evidence)")
    return 0
