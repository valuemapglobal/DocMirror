...
当前发行版本：DocMirror `1.1.1`；当前开发基线：`1.1.1`
...
- Python 包开发版本为 `1.1.1`，公共包名为 `docmirror`。
...
docmirror/
  ...
  plugins/           Seal 后领域 Projector、业务资源与统一 PluginRegistry 运行时
...
flowchart LR
    ...
    subgraph deliveryLayer["只读交付层"]
        ...
        sealed --> registry["Unified Post-Seal PluginRegistry"]
        registry --> community["Community Projector"]
        registry --> enterprise["Enterprise Projector"]
        registry --> finance["Finance Projector"]
...
2. Canonical Pipeline 是唯一允许修改事实的区域；其中不存在领域 Plugin、Provider、Registry 或 FactPatch。
3. 所有内置、商业和第三方 Plugins 都在 Seal 后由同一个 `PluginRegistry` 加载和执行，只能读取 `SealedParseResult`。
...
→ Generic Facts → Validation/Mutation Audit → Seal
```
...
## 9. Core 通用能力与 Post-Seal Plugins

### 9.1 内置 6+1 Community Projectors

当前随包提供七个 Seal 后 Community Projector：
...
标准落盘输出采用 Community Bundle 3.0。上述七类程序位于 `docmirror/plugins/<domain>`，通过统一 Post-Seal `PluginRegistry` 注册。它们基于 Seal 的只读副本派生领域事实、校验并组装 JSON，不向 Canonical 回写。`SealedParseResult` 是唯一事实 SSOT，Community、Enterprise、Finance 是从同一 Seal 独立产生的同级投影。
...
```text
docmirror/configs/domain/core_manifest.yaml
docmirror/configs/domain/resources/<domain>/
```

Seal 后 Plugin 投影逻辑、投影资源与统一注册：

```text
docmirror/plugins/<domain>/plugin.yaml
docmirror/plugins/<domain>/resources/
docmirror/plugins/_runtime/plugin_registry.py
```

### 9.2 插件执行原则

- 所有内置、商业和第三方插件只提交 Projector-only `PluginProvider`，并由唯一的 Post-Seal `PluginRegistry` 在 Seal 后注册。
- `plugin.yaml` 只声明 Provider、投影行为、投影输出和插件私有资源；不得包含 Core 的 `routing`、`classification`、`capabilities` 或 `dec_validation`。
- `EditionProjector` 只接收 `SealedParseResult`；商业 Projector 在自己的交付边界检查授权和 Sealed schema 兼容性。
- Plugin 可以基于 Seal 派生领域事实、执行领域校验、组装 Community/Enterprise/Finance JSON；其资源不得被 Seal 前 Core 读取。
