...
"""VLM (Vision-Language Model) OCR gateway routing — cloud API only.

Coordinates base64 image encoding and dispatches layout and text parsing
to the configured external VLM provider (OpenAI, Gemini, Anthropic, or
any OpenAI-compatible cloud API).

⚠️  Local VLM inference (Ollama, GGUF) is NOT supported.
    Cloud API providers only. See settings.py for configuration.
"""
