...
"""
Grid reconstruction — row/column clustering for OCR tables.

Purpose: Row grouping, x-clustering, Hough line detection, and 2D grid assembly
used by scanned page analysis.
...
def reconstruct_table_grid_from_tokens(
    tokens: list[OCRToken],
    page_h: float,
    hough_lines: list[tuple[float, float]] | None = None,
    y_tolerance: float = 8.0,
) -> list[list[str]]:
    """Reconstruct a 2-D table grid from an OCRToken stream (no re-OCR).

    Converts ``OCRToken`` objects into the char-dict format expected by the
    existing 2-D grid reconstruction pipeline, then delegates to
    ``_reconstruct_table_grid_2d``.

    Args:
        tokens: OCR tokens covering the table body region.
        page_h: Page height in image pixels (used for ``_split_tables_by_y_gap``).
        hough_lines: Optional pre-computed column boundaries from Hough detection.
        y_tolerance: Max vertical distance (px) to group chars into a row.

    Returns:
        2-D list of cell strings (the largest/most-prominent table grid),
        or empty list if reconstruction fails.
    """
    if not tokens:
        return []

    # OCRToken bbox: (x0, y0, x1, y1) — same coordinate semantics as the
    # char-dict "x0", "x1", "top", "bottom" fields used by the raw pipeline.
    chars = [
        {
            "x0": t.bbox[0],
            "x1": t.bbox[2],
    ...
    rows_by_y = _group_chars_into_rows(chars, y_tolerance=y_tolerance)
    if len(rows_by_y) < 2:
        return []
    ...
    for group in table_groups:
        ...
        tb = _reconstruct_table_grid_2d(grp_chars, hough_lines=hough_lines)
    ...
    # Return the table with the most rows (main table body)
