...
"""
OCR preprocess — full scanned-page image preparation pipeline.
...
Downstream: ``ocr.recognize.runner``, ``ocr.aistudio_provider``.
"""
...
def _resolve_external_ocr_provider(provider_spec: str | None):
    ...
    try:
        ...
        logger.debug("[external_ocr] Provider module not found: %s", provider_spec)
        return None
    except Exception as e:
        logger.warning("[external_ocr] Failed to resolve provider %r: %s", provider_spec, e)
        return None
...
def assess_image_quality_from_bgr(img_bgr) -> int:
    ...
    UDTR image quality scoring for consistency. Used to decide
