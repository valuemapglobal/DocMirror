...
from docmirror.runtime.optional_deps import require_optional_modules
...
def _ensure_image_deps() -> tuple[Any, Any]:
    """Load and cache image dependencies only when OCR preprocessing runs."""
    global cv2, np
    if cv2 is None or np is None:
        deps = require_optional_modules(["cv2", "numpy"], feature="OCR image preprocessing", extra="ocr")
        cv2 = deps["cv2"]
        np = deps["numpy"]
    return cv2, np
