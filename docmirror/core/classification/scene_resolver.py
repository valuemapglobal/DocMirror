...
def resolve_document_scene(
    ...
    *,
    ...
    Scoring: longest keyword match wins; confidence scales with keyword length vs sample.
    """
    text = text_sample or ""
    if not text.strip():
    ...
    for scene, spec in keywords_map.items():
        ...
        for kw in includes:
            if not kw or kw not in text:
                continue
            # Confidence from keyword specificity — NOT diluted by document length.
            # Long ledger bodies must not push title keywords below threshold.
