"""
Scanned document OCR Fallback (Scanned Page OCR Fallback)
=============================================

从 layout_analysis.py Split的Scanned documentProcessingModule。
contains analyze_scanned_page 及其Helper functions。
"""
...
def _preprocess_image_for_ocr(img_bgr):
    """OCR 前图像预Processing (增强版 v2):
    1. DPI Detect + 低Resolution自动放大
    2. 自适应 CLAHE 对比度增强
    3. 双边滤波 (边缘retain降噪)
    4. Unsharp Mask 锐化
    5. 形态学去噪 (remove小斑点)
    ...
    # ── Step 0: 低ResolutionDetect + 自动放大 ──
    # 如果Image太小 (短边 < 1000px), 2x 放大以提升 OCR 精度
    min_dim = min(h, w)
    if min_dim < 1000:
        scale = 2.0
        img_bgr = cv2.resize(
        ...
        h, w = img_bgr.shape[:2]
        logger.debug(f"[OCR] 低ResolutionImage已放大 2x → {w}x{h}")

    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)

    # ── Step 1: 自适应 CLAHE (based on对比度动态调整) ──
    contrast = gray.std()
    clip_limit = 3.0 if contrast < 40 else 2.0 if contrast < 80 else 1.5
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=(8, 8))
    gray = clahe.apply(gray)

    # ── Step 2: 双边滤波 (边缘retain降噪, 优于高斯) ──
    ...
    # ── Step 4: 自适应二值化 ──
    # blockSize=21 对Scanned documentTable效果更好 (比 15 更抗Background渐变)
    binary = cv2.adaptiveThreshold(
        gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        cv2.THRESH_BINARY, 21, 8,
    )

    # ── Step 5: 形态学去噪 (remove <3px 的小斑点噪声) ──
...
def _detect_table_lines_hough(
    ...
    """Hough 变换DetectScanned document中的Table垂直线。"""
    import cv2
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
...
def analyze_scanned_page(
    ...
    """对Scanned documentPageExecute OCR Extract。"""
    try:
        import numpy as np
        import cv2
        ...
        # ── Optimize: 图文混合先验 (Hybrid Text-Vision Prompt Prior) ──
        text_prior = ""
        if table_bbox:
        ...
        page_h = 0
        img = None
        for dpi in [200, 300]:  # M3: 首次 200 DPI (原 150), Retry 300
        ...
        # OCR Post-processing纠正 (Amount/Date/领域词典)
