...
from docmirror.tables.structure_detect import PipeGridSignal, detect_pipe_grid_in_text
...
def apply_pipe_enrich_spe(spe: dict[str, Any]) -> dict[str, Any]:
    """Mark SPE after section-led embedded pipe grid enrichment."""
    out = dict(spe)
    ...
    *,
    ...
    if ltqg_summary:
        ...
        if ltqg_summary.get("enabled"):
            ...
            raw = int(ltqg_summary.get("raw_max_rows") or 0)
            if raw:
                out["ltqg_raw_max_rows"] = raw
...
def apply_page_morphology_spe(
    ...
    *,
    ...
    from docmirror.models.mirror.vnext_page_projection import document_morphology_stats
