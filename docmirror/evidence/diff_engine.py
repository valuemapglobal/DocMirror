...
class DiffChange:
    ...
    kind: Literal[
        ...
        "review_changed",
        "coverage_changed",
        "quality_decision_changed",
        "needs_review_added",
        "needs_review_removed",
    ] = "node_changed"
...
class DiffReport:
    ...
    summary: dict[str, int] = field(
        default_factory=lambda: {
            ...
            "node_removed": 0,
            "node_changed": 0,
            "field_value_changed": 0,
            "confidence_changed": 0,
            "bbox_changed": 0,
            "edge_added": 0,
            "edge_removed": 0,
            "quality_decision_changed": 0,
            "total_changes": 0,
        }
    )
...
def diff_graphs(
    ...
    *,
    ...
    base_edges = {
        (e.get("from", e.get("from_node", "")), e.get("to", e.get("to_node", ""))): e for e in base.get("edges", [])
    }
    cand_edges = {
        (e.get("from", e.get("from_node", "")), e.get("to", e.get("to_node", ""))): e
            ...
            if bv != cv:
                ...
                report.changes.append(
                    DiffChange(
                        ...
                        before=f"{compare_field}: {bv}",
                        after=f"{compare_field}: {cv}",
                        source_refs=base_n.get("source_refs", []) or cand_n.get("source_refs", []),
                        visual_nodes=[nid],
                        page=base_n.get("page", 0),
                        message=f"Node {nid} {compare_field} changed: {bv} -> {cv}",
                    )
                )
                if compare_field == "bbox":
