...
from docmirror.configs.structure_policy import pipe_grid_veto_threshold
...
def read_ltqg_summary(
    ...
    if structure_spe and structure_spe.get("ltqg_enabled"):
        ...
        raw = int(structure_spe.get("ltqg_raw_max_rows") or 0)
        if raw:
            out["raw_max_rows"] = raw
...
def spe_ltqg_warnings(structure_spe: dict[str, Any] | None) -> list[str]:
    """Audit warnings when LTQG skipped tables or expected rows diverge from raw max."""
    ...
    raw_max = int(structure_spe.get("ltqg_raw_max_rows") or 0)
    if raw_max > expected and raw_max > 0:
        warnings.append(f"ltqg:expected_below_raw_max:{expected}/{raw_max}")
    return warnings


def mirror_api_meta_fields(parse_result: Any) -> dict[str, Any]:
    """Mirror Core SSOT fields merged into vNext mirror metadata."""
    ...
    if ltqg.get("enabled"):
        ...
        raw = int(ltqg.get("raw_max_rows") or 0)
        if raw:
            block["raw_max_rows"] = raw
