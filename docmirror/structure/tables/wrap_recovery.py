...
def normalize_punctuation(rows, type_col=None):
    ...
    for row in rows:
        for ci in range(len(row)):
            ...
            # 2. For type column: ASCII (CN) -> fullwidth (CN)
...
def complete_truncated_times(
    ...
    """Repair truncated timestamp (missing HH:MM:SS).

    WeChat payment trade_id contains time info:
    ...
    for row in rows:
        ...
        # Only date (YYYY-MM-DD), no time -> need to fill
        if re.match(r"^\d{4}-\d{2}-\d{2}$", time_val):
            trade_id = row[trade_id_col].strip()
            # Extract time from trade_id: format YYYYMMDDHHMMSS
            # Position: prefix(10) + date(8) + time(6)
            if len(trade_id) >= 24:
