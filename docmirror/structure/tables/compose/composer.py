...
from docmirror.structure.fusion import (
...
class TableComposer:
    ...
    def __init__(self, confidence_threshold: float = 0.7):
        from docmirror.structure.tables.cross_page_predictor import CrossPageTablePredictor
    ...
    def _compose_group(self, group: list[tuple[int, int, Any]], group_index: int) -> LogicalTable:
        ...
        merge_method = "cross_page_continuation" if len(source_pages) > 1 else "none"
        merge_confidence, merge_audit = self._score_table_row_merge(all_rows, source_pages)
        lid = logical_table_id(group_index)
        return LogicalTable(
            table_id=lid,
            logical_id=lid,
    ...
    def _score_table_row_merge(self, rows: list[TableRow], source_pages: list[int]) -> tuple[float, list[dict]]:
        """Score ParseResult PageContent cross-page merges, using bbox boundaries when available."""
        if len(source_pages) <= 1:
            return 1.0, []

        page_to_rows: dict[int, list[TableRow]] = {}
        for row in rows:
            page_to_rows.setdefault(int(row.source_page or source_pages[0]), []).append(row)

        audit: list[dict] = []
        scores: list[float] = []
        ordered_pages = [page for page in source_pages if page in page_to_rows]
        for i in range(1, len(ordered_pages)):
            prev_p = ordered_pages[i - 1]
            next_p = ordered_pages[i]
            validation = self._predictor.validate_table_row_boundary_merge(
                page_to_rows.get(prev_p, []),
                page_to_rows.get(next_p, []),
                prev_page_no=prev_p,
                next_page_no=next_p,
            )
            scores.append(validation.score)
