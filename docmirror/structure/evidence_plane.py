...
from docmirror.structure.normalization import (
    build_identity_trace,
    build_normalization_trace,
    rotation_matrix,
)
from docmirror.structure.normalization.orientation import candidate_from_metadata, orientation_comparison_signals
    ...
    def _from_parse_result(self, source: DocumentSource) -> EvidencePlane:
        ...
        if _pdf_path:
            try:
                ...
                if Path(pdf_path).exists():
                    ...
                    for page_record in pages:
                        ...
                        if _seal_detection_enabled(source.metadata, scene=source.metadata.get("scene", "")):
                            ...
                            from docmirror.structure.ocr.vision.seal_detector import detect_seals_hybrid
                            ...
                            for det in detections[:5]:  # max 5 per page
                                ...
                                seal_atom = EvidenceAtom(
                                    ...
                                    metadata={
                                        ...
                                        "detector": "docmirror.structure.ocr.vision.seal_detector.hybrid",
                ...
                _lg.getLogger("docmirror.structure.evidence_plane").debug("[EvPlane] %s", _diag_msg)
...
def _coordinate_transform(page_w: float | None, page_h: float | None, rotation: Any) -> dict[str, Any]:
    ...
    from docmirror.structure.normalization import invert_matrix
...
def _page_evidence_bundles(domain_specific: dict[str, Any]) -> list[dict[str, Any]]:
    try:
        from docmirror.structure.ocr.page_canvas.evidence_bundles import build_page_evidence_bundles
    except Exception:
