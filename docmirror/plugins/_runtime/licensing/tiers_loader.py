...
def premium_feature(domain: str) -> str:
    from docmirror.plugins._runtime.licensing.contract import premium_feature as _premium_feature

    return _premium_feature(domain)


def _enterprise_defined_premium_features() -> set[str]:
    """All premium features explicitly listed under the enterprise tier."""
    tiers = load_tiers().get("tiers") or {}
    entry = tiers.get("enterprise") or {}
    return set(entry.get("features") or [])
...
def tier_features(tier: str) -> list[str]:
    ...
    if entry.get("include_all_enterprise_domains"):
        features.update(_enterprise_defined_premium_features())
