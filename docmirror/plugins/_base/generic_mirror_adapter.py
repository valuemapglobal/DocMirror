...
from docmirror.ocr.correction.validators import validate_date_text, validate_phone_text
        ...
        for table in getattr(page, "tables", []) or []:
            for row in getattr(table, "rows", []) or []:
                ...
                if len(nonempty_cells) == 1:
                    ...
                    title = _clean_label(getattr(cell, "text", cell))
...
def build_generic_community_output(
    ...
    if page_count and not full_text.strip() and not fields and not records:
