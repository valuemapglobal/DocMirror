...
from docmirror.layout.columns import (
    ColumnLayout,
    detect_columns_from_pages,
)
from docmirror.topology.cross_page import (
    BridgeList,
    detect_cross_page_bridges,
)
from docmirror.topology.reading_order import (
    ReadingFlow,
    compute_reading_order,
)
from docmirror.topology.section_tree import (
    SectionNode,
    SectionTree,
    build_section_tree,
    detect_headings,
    ...
    def build(self, pages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self._config.get("column_detection", True):
            try:
                ...
                result["quality"]["column_detection_degraded"] = any(a.degraded for a in column_layout.pages.values())
