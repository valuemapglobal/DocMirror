...
"""Universal Block Index (UBI) for vNext page projections."""
...
from docmirror.topology.page_projection.models import PageBlock, PageRegion
...
def page_projection_blocks_enabled() -> bool:
    """Whether to materialize ``pages[n].blocks`` (Design 20 Phase 0)."""
    val = os.environ.get("DOCMIRROR_PAGE_PROJECTION_BLOCKS", "1")
    return val not in ("0", "false", "False", "")


def morphology_orchestrator_enabled() -> bool:
    """Whether Morphology Orchestrator gate is active (Design 20 Phase 1)."""
    val = os.environ.get("DOCMIRROR_PAGE_PROJECTION_MO", "1")
    return val not in ("0", "false", "False", "")
...
def _schema_hint_for_region(region: PageRegion, *, document_type: str | None = None) -> str:
    ...
    try:
        from docmirror.ocr.structure_project import infer_schema_hint_v2
...
def reading_order_refs_from_blocks(blocks: list[PageBlock]) -> list[str]:
    """Return page-local reading refs for regions, texts, tables, and key-values."""
    ordered = sorted(blocks, key=_block_sort_key)
    refs: list[str] = []
...
def build_page_blocks(
    ...
    *,
    ...
    """Derive UBI blocks, morphology summary, and reading order."""
    blocks: list[PageBlock] = []
