...
"""Morphology Orchestrator (MO) for vNext page projection materialization."""
...
from docmirror.topology.page_projection.block_index import morphology_orchestrator_enabled
from docmirror.topology.page_projection.models import PageBlock, PageFlow, PageRegion
...
def _should_skip_field_materialize(*, content_type: str | None, page_has_tables: bool) -> bool:
    ...
    if not morphology_orchestrator_enabled():
        return False
...
def orchestrate_page_morphology(
    ...
    *,
    ...
    from docmirror.ocr.structure_project import assign_schema_hints_to_regions
    from docmirror.topology.page_projection.block_index import (
        build_page_blocks,
        reading_order_refs_from_blocks,
    )
    ...
    return MorphologyOrchestratorResult(
        ...
        reading_order_refs=reading_order_refs_from_blocks(blocks),
...
def materialized_region_ids_from_bundle(bundle: dict[str, Any]) -> set[str]:
    """Map persisted L1 structures in a page bundle to projection region ids."""
    ids: set[str] = set()
