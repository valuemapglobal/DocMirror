...
from docmirror.tables.statement import build_statement_structure
from docmirror.topology.page import TopologyRegion
from docmirror.topology.reconstruction_contract import ReconstructionContract, default_contract
...
def _attach_dispatch_provenance(
    ...
    *,
    ...
    contract = reconstructor.contract() if hasattr(reconstructor, "contract") else default_contract(reconstructor)
    candidates = [
        {
            "reconstructor": getattr(candidate, "id", type(candidate).__name__),
            "score": round(float(candidate_score), 6),
            "contract_id": (
                candidate.contract().id if hasattr(candidate, "contract") else default_contract(candidate).id
            ),
        }
...
class FinancialStatementReconstructor:
    ...
    def score(self, region: TopologyRegion, context: ReconstructionContext) -> float:
        ...
        statement_keywords = (
            diagnostics.get("statement_keywords") if isinstance(diagnostics.get("statement_keywords"), list) else []
        )
...
def _table_region_geometry(region: TopologyRegion) -> dict[str, object]:
    keys = (
        "geometry_source",
        "geometry_confidence",
        "coordinate_system",
...
def _line_groups(atoms: list[EvidenceAtom]) -> list[list[EvidenceAtom]]:
    ...
    ordered = sorted(
        with_bbox, key=lambda atom: (float((atom.bbox or [0, 0, 0, 0])[1]), float((atom.bbox or [0, 0, 0, 0])[0]))
    )
...
def _headers(atoms: list[EvidenceAtom]) -> list[str]:
    headers_by_index = {
        int(atom.metadata["header_index"]): str(atom.text or "") for atom in atoms if "header_index" in atom.metadata
    }
...
def _cell_dict(
    ...
    *,
    ...
    out: dict[str, object] = {
        ...
        "bbox": bbox,
        "evidence_ids": source_evidence_ids or evidence_ids,
    ...
    if atom:
        for source_key, output_key in (
            ...
            value = atom.metadata.get(source_key)
            if value not in (None, "", [], {}):
                out[output_key] = value
    return out
