...
"""Phase 4+ PCM mirror API contract."""
...
def test_pcm_legacy_ref_gate_script():
    import subprocess
    import sys
    ...
    proc = subprocess.run(
        [sys.executable, str(root / "tools" / "gate_pcm_legacy_refs.py")],
        cwd=root,
        capture_output=True,
        text=True,
    )
...
def test_pcm_mirror_volume_gate_script():
    import subprocess
    import sys
    from pathlib import Path
    ...
    root = Path(__file__).resolve().parents[2]
    proc = subprocess.run(
        [sys.executable, str(root / "tools" / "gate_pcm_mirror_volume.py")],
        cwd=root,
        capture_output=True,
        text=True,
    )
