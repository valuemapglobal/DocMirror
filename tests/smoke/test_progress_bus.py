...
# ── Post-Extract Catalog Validation ───────────────────────────────────────


class TestPostExtractCatalog:
    """Post-extract hook catalog is correctly configured."""

    def test_post_extract_yaml_exists(self):
        """post_extract.yaml config file exists."""
        from docmirror.configs.paths import POST_EXTRACT_YAML

        assert POST_EXTRACT_YAML.is_file(), f"Not found: {POST_EXTRACT_YAML}"

    def test_post_extract_catalog_loads(self):
        """post_extract catalog loads without errors."""
        from docmirror.plugins._runtime.post_extract.catalog import load_post_extract_catalog

        catalog = load_post_extract_catalog()
        assert len(catalog) > 0, "Post-extract catalog is empty"

    def test_post_extract_hooks_importable(self):
        """All post-extract hook modules are importable."""
        from docmirror.plugins._runtime.post_extract.catalog import validate_post_extract_catalog

        errors = validate_post_extract_catalog()
        assert len(errors) == 0, f"Post-extract catalog errors: {errors}"
    ...
    def test_post_extract_hook_module_paths(self):
        ...
        for hook_id, spec in hooks.items():
            ...
            assert module.startswith("docmirror.plugins._runtime.post_extract.hooks."), (
                f"Hook {hook_id} has incorrect module path: {module}"
            )
