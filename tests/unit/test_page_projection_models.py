...
from docmirror.topology.page_projection.models import PageFlow, PageProjection, PageRegion
...
def test_page_projection_to_dict():
    canvas = PageProjection(
        page_number=4,
        width=100.0,
        height=200.0,
...
def test_vnext_mirror_omits_removed_document_fields():
    grid = {
        "grid_id": "mg_p4_repayment_0",
        "page": 4,
        "bbox": [1, 2, 3, 4],
    ...
    assert "page_projection_removed_shim" not in api
    page = get_page_projection(doc, 4)
    assert page is not None
...
def test_page_access_ignores_removed_micro_grids_field():
