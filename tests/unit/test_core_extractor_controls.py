...
def test_vnext_extractor_suppresses_text_owned_by_scanned_table(monkeypatch):
    ...
    def fake_ocr(_file_path, _page_index, page_number, *, start_order=0):
        return [
            Block(block_id=f"ocr:{page_number}:title", block_type="text", raw_content="表格标题", page=page_number, evidence_ids=(f"ocr:{page_number}:title",)),
            Block(block_id=f"ocr:{page_number}:row0", block_type="text", raw_content="项目", page=page_number, evidence_ids=(f"ocr:{page_number}:row0",)),
            Block(block_id=f"ocr:{page_number}:row1", block_type="text", raw_content="货币资金", page=page_number, evidence_ids=(f"ocr:{page_number}:row1",)),
        ]
