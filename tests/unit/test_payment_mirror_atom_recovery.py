...
def test_alipay_recovers_only_complete_issuer_rows_from_mirror_atoms():
    headers = [
        ("收/支", 51.0),
        ("交易对方", 79.0),
        ("商品说明", 134.0),
    ...
    recovered = plugin._recover_records_from_mirror(_parse_result(atoms))
    ...
    assert built[0]["source"]["source"] == "mirror_text_atoms"
...
def test_alipay_recovers_complete_narrative_identity_without_sentence_suffixes():
    ...
    recovered = AlipayPaymentPlugin()._recover_identity_from_mirror(_parse_result(atoms))
...
def test_wechat_recovers_complete_narrative_identity_without_sentence_suffixes():
    ...
    recovered = WeChatPaymentPlugin()._recover_identity_from_mirror(_parse_result(atoms))
