...
"""Canonical assembly uses merger quarantine + LTQG export pipeline."""
...
def test_canonical_composition_preserves_quarantined_merge_groups_in_mirror():
    headers = [
        "交易日期",
        "摘要",
        "借方发生额",
...
def test_canonical_recovery_differs_from_previous_compose_heuristic():
    ...
    pr = assemble_parse_result(pages, {}, "")
    assert len(pr.logical_tables) == 2
    assert pr.logical_tables[1].merge_method == "quarantine_standalone"

    parse_pages = assemble_parse_result(pages, {}, "").pages
